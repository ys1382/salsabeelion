...
- [ ] **Week one after Monday: forest wood before café** (owner — 2026-09-23) — Day 1 stays café. Later week-one days: head into the forest first and cut parts of **one dead tree** (same shape, dead-brown needles). Prefer the existing **stump** after the cuts (fewer branches TBD). Carry logs in a **sack from home**. End of day: Mara says it is chilly tonight; at home, build/light a **campfire behind the house**, far from other buildings. Use the pack’s **lit campfire** flicker (not an invented flame); unlit pit first if needed. Keep it outdoor — never indoors.
