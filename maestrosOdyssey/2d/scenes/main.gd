extends Node2D
# Entry point. Loads the authored Dragon's Brew neighborhood and plays it.
#
# Pressing Play in the editor takes that path. Flags:
#
#   godot --path .                                        # authored café
#   godot --path . -- --world backend/fallback_world.json # old zij village
#   godot --path . -- --generate --online                 # LLM village (opt-in)
...
## Order of preference: a file named on the command line, then the authored
## Dragon's Brew neighborhood. Generation is opt-in (--generate --online).
