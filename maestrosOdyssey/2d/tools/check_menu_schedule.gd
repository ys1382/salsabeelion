...
func _initialize() -> void:
	...
	var mon_prompt: String = cafe.order_prompt()
	assert(mon_prompt.begins_with("What's your order?"))
	assert(mon_prompt.contains("A drink and a food"))
	assert(not mon_prompt.contains("this morning"), mon_prompt)
	assert(not mon_prompt.contains("café"), mon_prompt)
	assert(not mon_prompt.contains("pesos"), mon_prompt)
	...
	assert(not tue_prompt.contains("this morning"), tue_prompt)
	assert(not tue_prompt.contains("chocolate"), tue_prompt)
	assert(not tue_prompt.contains("pesos"), tue_prompt)
	...
	assert(not wed.contains("this morning"), wed)
	assert(not wed.contains("tostada"), wed)
	assert(not wed.contains("pesos"), wed)
	...
	assert(not thu.contains("this morning"), thu)
	assert(not thu.contains("muffin"), thu)
	assert(not thu.contains("pesos"), thu)
	...
	assert(put.contains("blueberries"), put)
	assert(put.contains("lonely"), put)
	assert(not put.contains("Adiós"), put)
	assert(cafe.visit_hint)
	assert(cafe.take_visit_hint())
	var bye_line: String = cafe.cart_goodbye_line()
	assert(bye_line.contains("Adiós, and buenas noches"), bye_line)
	assert(bye_line.contains("good night"), bye_line)
