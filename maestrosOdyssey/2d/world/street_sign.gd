...
## On House_Hay_2, centered on the grey attic wall above the ground door.
const CAFE_BOARD_AT := Vector2(33, -34)
...
class HangingBoard extends Node2D:
	func _draw() -> void:
		...
		var board := Vector2(maxf(widest + 4.0, 22.0), line_h * lines.size() + 2.0)
		var top := Vector2(-board.x * 0.5, -board.y)
		draw_rect(Rect2(top + Vector2(-1, -1), board + Vector2(2, 2)), sign.WOOD_DARK)
		...
		var y := top.y + 2.0 + font.get_ascent(sign.FONT_SIZE)
		for line in lines:
			var w := font.get_string_size(line, HORIZONTAL_ALIGNMENT_LEFT, -1, sign.FONT_SIZE).x
			var x := top.x + (board.x - w) * 0.5
			draw_string(font, Vector2(x, y), line, HORIZONTAL_ALIGNMENT_LEFT, -1, sign.FONT_SIZE, sign.INK)
			y += line_h
