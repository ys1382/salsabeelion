...
- **Focus mode picker** — Walk / wild, Garden, **Hiking**, **Seashore**, **Crops & Domestic Animals** (internal key `food`). Hiking / Seashore = engaged trail-walker / beachgoer stances (people–place relationship lean; not GPS; other modes may still mention relationship). Crops mode: Claude may do domestication/crop timelines and noticing; **must not** write human-injustice history (famines-as-policy, settler–Indigenous conflict) — those need owner-curated sources later. Environmental complications unlock with **fact level**. Pigs = husbandry/ancestry only (never encourage eating; health claims later). Separate fact pools per mode.
