...
- [x] **Bookstore inventory aggregation scaffold (Aug 2026)** — Modular adapters + SQLite models + matching/freshness + Owner’s Office dashboard + Wishlist “Available from bookstores” UI. Starter configs: Barnes & Noble, Kepler’s, Green Apple (**paused** — robots disallow catalog search/books crawl; ISBN product-page checks only when enabled). Fixture adapter + tests. Docs: `bookstore_inventory/README.md`. Live enable needs per-store legal/feed review.
