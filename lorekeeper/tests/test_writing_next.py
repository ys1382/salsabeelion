...
class WritingNextAnswerTests(unittest.TestCase):
    ...
    def test_leave_off_adjacent_ranks_higher(self):
        entries = [
            _entry(
                "n1",
                "Far future",
                "Years later the empire collapses after the treaty fails.",
            ),
            _entry(
                "n2",
                "Near now",
                "Character E dreads what Lord Character T intends after the mountain path.",
            ),
            _entry(
                "d1",
                "Draft",
    ...
    def test_compose_shape(self):
        ...
        self.assertIn("•", out)
        self.assertIn("notes vs draft only", out.lower())
