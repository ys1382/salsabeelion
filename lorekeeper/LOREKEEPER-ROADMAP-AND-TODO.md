...
- [x] **Raise gold floor (owner 2026-08-12 afternoon)** — newer catch-up voice (Fell / Author vision / registration / Silver-Heart framing) raised into `catchup_waking_dream_gold.txt`, **keeping** prior good Sparrow entry-reason + can’t-leave-yet beats (gold-pin rule: don’t drop entry-reason beats that were already good). Cursor rule: `.cursor/rules/lorekeeper-ask-gold-pin.mdc`; playbook section “Pinning a gold Ask sample.”
- [x] **Sparrow domain-entry reason (owner 2026-08-12 afternoon)** — catch-up must-keep when saved: Sparrow (or equivalent) as why she’s in the antagonist domain (offer/premise), plus why she can’t leave too quickly. Do **not** invent transport mechanics. Prompt + local gather + synthetic tests.
