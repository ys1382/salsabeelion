...
- [x] **Gold BASELINE locked (2026-08-12; raised same day to owner “perfect” sample)** — Q: *Get me caught up with this story.* Owner: **exactly what I needed** for premise / thin-draft catch-up (not a fully plotted novel recap). Machine lock: `lorekeeper/tests/fixtures/catchup_waking_dream_gold.txt` + `test_owner_gold_baseline_shape_locked`. **Do not regress** this voice or must-keeps: Fall/Author origin · named antagonist sparse framing · **antagonist softer side / not without a soul** · **Sparrow brought her there** (entry reason, not nearby-only) · **cannot simply leave** / registration / unmask risk · paranoia vs real threat · task of getting out without discovery · footer invent-nothing. Continuous planning-brief prose (not cast-card bullets). Later answers may add more if notes deepen; they must not lose this bar. Gold-pin rule: don’t drop entry-reason / can’t-leave / softer-side beats that were already good.
